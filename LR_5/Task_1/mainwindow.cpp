...
MainWindow::MainWindow(QWidget* parent)
    ...
    // Install event filter for space key
    ui->pushButton_58->installEventFilter(this);
...
bool MainWindow::eventFilter(QObject* obj, QEvent* event) {
    if (obj == ui->pushButton_58 && event->type() == QEvent::KeyPress) {
        QKeyEvent* keyEvent = static_cast<QKeyEvent*>(event);
        if (keyEvent->key() == Qt::Key_Space) {
            if (isWork && !text.isEmpty() && text.at(0) == ' ') {
                text.remove(0, 1);
                correctSymbol++;
            } else if (isWork) {
                uncorrectSymbol++;
...
void MainWindow::updateText(int number) {
    ...
    case 2: {
        text = "FamilieMüllerplantihrenUrlaubSiegehtineinReisebüroundlässtsichvon";
        break;
    }
    case 3: {
        text = "JeappellenglicaSummerj'aiansjecanadiennemafamilletmoivonsdanslesudFrance.";
        break;
    }
    case 4: {
...
void MainWindow::keyPressEvent(QKeyEvent* event) {
    ...
    // Skip space handling here as it's now handled by eventFilter
